...
//	1.bugs collected during five days
	...
	int	speed,hour,D,year,yearnum=1,days,psum=0,q,r,rsum=0,myear=1,msum=0,cm=1,yday;
	float mm=1.6,wam=8000,calories,pound=0.0002666667,calmonth,tcal,organ,inch,inches=0;
...
//	2.Number of burned calories
	cout<<"\n\nQuestion No Two "<<endl;
	cout<<"Number of Burnded Calories "<<endl;
	for(int j=1; j<=5; j++)
	{	cout<<"Number of Calories Burned after "<<mnt<<" Minutes are "<<mnt*4.2<<endl;	sleep(1);
		mnt+=5;	
	}	cout<<endl;
//	3.Budget Analysis
	cout<<"\n\nQuestion No three "<<endl;	
	...
	else {	cout<<"your Entered value is Invalid that is not  in Positive Range"<<endl;}
...
//	4.Distance Traveled
	...
	{	D=speed*hour;
		cout<<"Traveled Distance in "<<hour<<" hours is "<<D<<" Miles "<<endl; sleep(1);
		--hour;
	}
	}
	else
	{	cout<<"your Value is Ivalid ";	}
	*/
//	5.Average Rainfall
	cout<<"\n\nQuestion No Five "<<endl;
	cout<<"Enter the Number of Years "<<endl;				cin>>year;
	for(float n=1; n<=year; n++)
	{		cout<<yearnum<<" year "<<endl;
		for(float o=1; o<=12; o++)
		{
...
//	6.Pennies for pay
	...
	cout<<"Enter the Number of Days, and the program will say you  your money from first day in pennies  "<<endl; 			cin>>days;
	for(int p=1; days!=0; p++)
	{	cout<<" day "<<p<<endl; 	sleep(1);
		psum=psum+days;		--days; 
	}
...
//	11.Weight loss
	...
	{	tcal=calories*pound*cm*30;	
		cout<<" decreasing "<<calories<<" calories on each day for "<<cm<<" Months  the loss weight is "<<tcal<<" Pound(s) "<<endl; sleep(1);
	}
//	12.
	cout<<"\n\nQuestion No twelewe"<<endl;
	...
	{	organ=organ+30*(organ/100);
		cout<<left<<setw(5)<<"  "<<yday<<"               "<<left<<setw(20)<<organ<<endl;	sleep(1);
		++yday;	 
	}                                    
//	13.
